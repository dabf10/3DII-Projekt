...
RWStructuredBuffer<float> gMaximumLum : register( u1 );
...
void DownScale1024to4( uint dispatchThreadID, uint groupThreadID, inout float avgLum, inout float maxLum )
{
	...
	// step1 -> 1 -> 4 -> 16 and so on
	// step2 -> 2 -> 8 -> 32 and so on
	// step3 -> 3 -> 12 -> 48 and so on
	[unroll]
...
void DownScale4to1( uint dispatchThreadID, uint groupThreadID, uint groupID, float avgLum, float maxLum )
{
	...
	if (groupThreadID == 0)
	{
		...
		gAverageLum[groupID] = finalAvgLum;
		gMaximumLum[groupID] = maxLum;
	}
}
...
void DownScaleSecondPass(uint3 dispatchThreadID : SV_DispatchThreadID)
{
	...
	uint step1 = 1; // 1 -> 4 -> 16 and so on
	uint step2 = 2; // 2 -> 8 -> 32 and so on
	uint step3 = 3; // 3 -> 12 -> 48 and so on
	[unroll]
	...
	// When the the 1D texture has been downscaled to 4 texels, the very first
	// thread sums up those texels just like before, but instead of storing the
	// value we divide by the number of groups to get average and return it.
	if (dispatchThreadID.x == 0)
	{
		...
		gAverageLum[0] = exp( max( avgLum / gGroupCount, 0.0001 ) );
		gMaximumLum[0] = maxLum;
	}
}
